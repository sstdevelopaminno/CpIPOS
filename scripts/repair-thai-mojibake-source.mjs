...
const ROOT = path.resolve(process.cwd(), "apps/backoffice-web/src");
const EXCLUDED = new Set([
  path.resolve(ROOT, "components/system/thai-mojibake-recovery.tsx")
]);
...
function suspiciousScore(value) {
  ...
  for (const character of value) {
    ...
    if ((codePoint >= 0x80 && codePoint <= 0x9f) || codePoint === 0x00a0 || WINDOWS_1252_BYTES.has(codePoint)) {
      score += 2;
    }
  }
  return score;
...
function walk(directory, output = []) {
  for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
    ...
    else if (entry.isFile() && EXTENSIONS.has(path.extname(entry.name)) && !EXCLUDED.has(fullPath)) output.push(fullPath);
  }
  return output;
}
...
for (const filePath of walk(ROOT)) {
  ...
  console.log(`${path.relative(process.cwd(), filePath)}: repaired ${fileChangedLines} line(s)`);
}

console.log(`Thai mojibake source repair: ${changedFiles} file(s), ${changedLines} line(s) changed.`);
