...
2026-08-25 P0 POS stability checkpoint: FG0003/Android POS mitigation reduces client polling and fan-out in the Web POS sales surface, keeps native customer-display polling disabled on single-display diagnostics, stabilizes More/Payment menu visibility through resolved POS roles, and disables legacy `apps/backoffice-web` prebuild Python source mutation. No DB migration or live Supabase write is implied.
...
- The `/preview/pos/more` page title should read `More` / `เพิ่มเติม` and should not show a Back to Sales button; users return through the main POS navigation.
- Each moved page must keep a deterministic Back to More button linking to `/preview/pos/more`.
- Sales List defaults to the daily quick range. The totals cards are hidden by default and opened with the `View Totals` / `ดูยอด` button in the header.
