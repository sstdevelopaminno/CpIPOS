...
function getSellableStockStatus(stockUnits: number | null | undefined, isActive: boolean, th: boolean): SellableStockStatus {
  if (!isActive) {
    return { label: th ? "ปิดการขาย" : "Inactive", className: "border-slate-300 bg-slate-100 text-slate-700" };
  }
  if (stockUnits === null || stockUnits === undefined) {
    return { label: th ? "ไม่มีสูตร" : "No recipe", className: "border-slate-300 bg-slate-100 text-slate-700" };
  }
  if (stockUnits === 0) {
    return { label: th ? "หมด" : "Out of stock", className: "border-red-300 bg-red-100 text-red-800" };
  }
  if (stockUnits < 30) {
    return { label: th ? "ใกล้หมด" : "Nearly out", className: "border-yellow-300 bg-yellow-100 text-yellow-800" };
  }
  if (stockUnits < 100) {
    return { label: th ? "สต๊อกต่ำ" : "Low stock", className: "border-orange-300 bg-orange-100 text-orange-800" };
  }
  return { label: th ? "พร้อมขาย" : "Ready", className: "border-emerald-300 bg-emerald-50 text-emerald-800" };
}
...
export function StockProductsTable({
  ...
  const ingredientsSorted = useMemo(
    () => [...ingredientList].sort((a, b) => a.name.localeCompare(b.name, th ? "th" : "en", { sensitivity: "base" })),
    [ingredientList, th]
  );
  ...
  const filteredProducts = useMemo(() => {
    ...
    return baseProductsByMode.filter((item) => {
      ...
      return `${item.name} ${item.sku ?? ""} ${item.category ?? ""}`.toLowerCase().includes(keyword);
    });
  ...
  const filteredIngredients = useMemo(() => {
    ...
    return ingredientsSorted.filter((item) => {
      ...
      if (!keyword) return true;
      return `${item.name} ${item.baseUnit}`.toLowerCase().includes(keyword);
    });
  ...
  useEffect(() => {
    setToolbarTarget(document.getElementById("stock-page-action-toolbar"));
  }, []);
  ...
  const activeFilterCount =
    (modeFilter === "ingredients"
      ? Number(ingredientSearchText.trim().length > 0) + Number(ingredientUnitFilter !== "all")
      : Number(productSearchText.trim().length > 0) + Number(productCategoryFilter !== "all")) + Number(modeFilter !== "all");
  ...
  useEffect(() => {
    if (productCategoryFilter !== "all" && !productCategoryOptions.includes(productCategoryFilter)) setProductCategoryFilter("all");
  ...
  function openBulkDeletePopup(scope: "all" | "selected") {
    ...
    const targetIds = isIngredientsMode
      ? scope === "all" ? filteredIngredients.map((item) => item.id) : selectedFilteredIngredientIds
      : scope === "all" ? filteredProducts.map((item) => item.id) : selectedFilteredProductIds;
  ...
  async function submitBulkUnlinkRecipes() {
    ...
    try {
      ...
      setNotice(th ? `ยกเลิกผูกวัตถุดิบแล้ว ${updatedCount} รายการ${failedCount > 0 ? `, ไม่สำเร็จ ${failedCount}` : ""}` : `Unlinked recipes for ${updatedCount} products${failedCount > 0 ? `, ${failedCount} failed` : ""}.`);
  ...
  async function submitBulkDelete() {
    ...
    const ids = isIngredientsMode
      ? bulkDeleteScope === "all" ? filteredIngredients.map((item) => item.id) : selectedFilteredIngredientIds
      : bulkDeleteScope === "all" ? filteredProducts.map((item) => item.id) : selectedFilteredProductIds;
    if (ids.length === 0) {
    ...
    try {
      ...
      if (isIngredientsMode) {
        ...
        setNotice(th ? `ลบวัตถุดิบแล้ว ${deletedCount} รายการ${archivedCount > 0 ? ` (archive ${archivedCount})` : ""}` : `Deleted ${deletedCount} ingredients${archivedCount > 0 ? ` (${archivedCount} archived)` : ""}.`);
  ...
  function formatIngredientQty(item: IngredientItem) {
    ...
    if (unit === "piece" || unit === "unit" || unit === "ลูก") return String(Math.max(0, Math.floor(qty)));
  ...
  function getIngredientStatus(item: IngredientItem): SellableStockStatus {
    ...
    if (qty <= 0) return { label: th ? "หมด" : "Out", className: "border-red-300 bg-red-100 text-red-800" };
    if (qty <= nearOutThreshold) return { label: th ? "ใกล้หมด" : "Nearly out", className: "border-yellow-300 bg-yellow-100 text-yellow-800" };
    if (qty <= lowStockThreshold) return { label: th ? "สต๊อกต่ำ" : "Low stock", className: "border-orange-300 bg-orange-100 text-orange-800" };
    return { label: th ? "พร้อมใช้" : "Ready", className: "border-emerald-300 bg-emerald-50 text-emerald-800" };
  }
    ...
    <div className="min-w-0">
      ...
      {canManageCatalog && (modeFilter === "ingredients" ? selectedFilteredIngredientIds.length > 0 : selectedFilteredProductIds.length > 0) ? (
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2">
          <p className="text-xs font-bold text-slate-600">
            {modeFilter === "ingredients"
              ? th ? `เลือกวัตถุดิบแล้ว ${selectedFilteredIngredientIds.length} รายการ` : `${selectedFilteredIngredientIds.length} ingredients selected`
              : th ? `เลือกสินค้าแล้ว ${selectedFilteredProductIds.length} รายการ` : `${selectedFilteredProductIds.length} products selected`}
          </p>
          <div className="flex flex-wrap gap-2">
            {modeFilter !== "ingredients" ? (
              <button type="button" onClick={() => openBulkUnlinkPopup("selected")} className="inline-flex min-h-9 items-center rounded-lg border border-sky-200 bg-white px-3 text-xs font-bold text-sky-700 hover:bg-sky-50">
                {th ? `ยกเลิกผูกวัตถุดิบที่เลือก (${selectedFilteredProductIds.length})` : `Unlink Selected (${selectedFilteredProductIds.length})`}
              </button>
            ) : null}
            <button type="button" onClick={() => openBulkDeletePopup("selected")} className="inline-flex min-h-9 items-center rounded-lg border border-red-200 bg-white px-3 text-xs font-bold text-red-700 hover:bg-red-50">
              {modeFilter === "ingredients"
                ? th ? `ลบวัตถุดิบที่เลือก (${selectedFilteredIngredientIds.length})` : `Delete Selected Ingredients (${selectedFilteredIngredientIds.length})`
                : th ? `ลบ/ปิดการขายสินค้าที่เลือก (${selectedFilteredProductIds.length})` : `Delete/Deactivate Selected Products (${selectedFilteredProductIds.length})`}
            </button>
            <button type="button" onClick={() => openBulkDeletePopup("all")} className="inline-flex min-h-9 items-center rounded-lg border border-amber-300 bg-amber-50 px-3 text-xs font-bold text-amber-800 hover:bg-amber-100">
              {modeFilter === "ingredients"
                ? th ? `ลบวัตถุดิบทั้งหมด (${filteredIngredients.length})` : `Delete All Ingredients (${filteredIngredients.length})`
                : th ? `ลบ/ปิดการขายสินค้าทั้งหมด (${filteredProducts.length})` : `Delete/Deactivate All Products (${filteredProducts.length})`}
            </button>
          </div>
        </div>
      ) : null}
        ...
        <div className="fixed inset-0 z-[165] grid place-items-center bg-slate-900/35 p-4" onClick={() => setFilterPopupOpen(false)}>
          <div className="w-full max-w-2xl rounded-2xl border border-slate-200 bg-white p-4 shadow-2xl" onClick={(event) => event.stopPropagation()}>
            <div className="mb-3 flex items-center justify-between gap-3">
              ...
              <button type="button" onClick={() => setFilterPopupOpen(false)} className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs font-bold text-slate-700 hover:bg-slate-50">{th ? "ปิด" : "Close"}</button>
            </div>
            {modeFilter === "ingredients" ? (
              <div className="grid gap-3 sm:grid-cols-[1fr_180px]">
                <label className="grid gap-1 text-xs font-semibold text-slate-600">
                  <span>{th ? "ค้นหาวัตถุดิบ" : "Search Ingredients"}</span>
                  <input value={ingredientSearchText} onChange={(event) => { setIngredientSearchText(event.target.value); setCurrentPage(1); }} placeholder={th ? "พิมพ์ชื่อวัตถุดิบหรือหน่วย..." : "Type ingredient name or unit..."} className="min-h-10 rounded-lg border border-slate-300 bg-white px-3 text-sm text-slate-900" />
                </label>
                <label className="grid gap-1 text-xs font-semibold text-slate-600">
                  <span>{th ? "กรองหน่วย" : "Unit Filter"}</span>
                  <select value={ingredientUnitFilter} onChange={(event) => { setIngredientUnitFilter(event.target.value); setCurrentPage(1); }} className="min-h-10 rounded-lg border border-slate-300 bg-white px-3 text-sm text-slate-900">
                    <option value="all">{th ? "ทุกหน่วย" : "All units"}</option>
                    {INGREDIENT_BASE_UNIT_OPTIONS.map((option) => <option key={option.value} value={option.value}>{th ? option.thLabel : option.enLabel}</option>)}
                  </select>
                </label>
              </div>
            ) : (
              <div className="grid gap-3 sm:grid-cols-[1fr_220px]">
                <label className="grid gap-1 text-xs font-semibold text-slate-600">
                  ...
                  <input value={productSearchText} onChange={(event) => { setProductSearchText(event.target.value); setCurrentPage(1); }} placeholder={th ? "พิมพ์ชื่อสินค้า SKU หรือหมวดหมู่..." : "Type product name, SKU, or category..."} className="min-h-10 rounded-lg border border-slate-300 bg-white px-3 text-sm text-slate-900" />
                </label>
                <label className="grid gap-1 text-xs font-semibold text-slate-600">
                  <span>{th ? "กรองหมวดหมู่" : "Category Filter"}</span>
                  <select value={productCategoryFilter} onChange={(event) => { setProductCategoryFilter(event.target.value); setCurrentPage(1); }} className="min-h-10 rounded-lg border border-slate-300 bg-white px-3 text-sm text-slate-900">
                    <option value="all">{th ? "ทุกหมวดหมู่" : "All categories"}</option>
                    {productCategoryOptions.map((categoryName) => <option key={categoryName} value={categoryName}>{categoryName}</option>)}
                  </select>
                </label>
              </div>
            )}
            <div className="mt-4 flex flex-wrap justify-between gap-2 border-t border-slate-100 pt-3">
              <div className="flex flex-wrap gap-2">
                {modeFilter !== "ingredients" ? (
                  <button type="button" onClick={() => openBulkUnlinkPopup(selectedFilteredProductIds.length > 0 ? "selected" : "all")} disabled={filteredProducts.length === 0} className="inline-flex min-h-9 items-center rounded-lg border border-sky-200 bg-sky-50 px-3 text-xs font-bold text-sky-700 hover:bg-sky-100 disabled:cursor-not-allowed disabled:opacity-50">
                    {th ? (selectedFilteredProductIds.length > 0 ? `ยกเลิกผูกวัตถุดิบที่เลือก (${selectedFilteredProductIds.length})` : `ยกเลิกผูกวัตถุดิบทั้งหมด (${filteredProducts.length})`) : (selectedFilteredProductIds.length > 0 ? `Unlink Selected Recipes (${selectedFilteredProductIds.length})` : `Unlink All Recipes (${filteredProducts.length})`)}
                  </button>
                ) : null}
                <button type="button" onClick={() => openBulkDeletePopup("selected")} disabled={modeFilter === "ingredients" ? selectedFilteredIngredientIds.length === 0 : selectedFilteredProductIds.length === 0} className="inline-flex min-h-9 items-center rounded-lg border border-amber-500 bg-amber-500 px-3 text-xs font-bold text-white hover:bg-amber-600 disabled:cursor-not-allowed disabled:opacity-50">
                  {modeFilter === "ingredients" ? (th ? `ลบที่เลือก (${selectedFilteredIngredientIds.length})` : `Delete Selected (${selectedFilteredIngredientIds.length})`) : (th ? `ปิดการขายที่เลือก (${selectedFilteredProductIds.length})` : `Deactivate Selected (${selectedFilteredProductIds.length})`)}
                </button>
                <button type="button" onClick={() => openBulkDeletePopup("all")} disabled={modeFilter === "ingredients" ? filteredIngredients.length === 0 : filteredProducts.length === 0} className="inline-flex min-h-9 items-center rounded-lg border border-amber-200 bg-white px-3 text-xs font-bold text-amber-700 hover:bg-amber-50 disabled:cursor-not-allowed disabled:opacity-50">
                  {modeFilter === "ingredients" ? (th ? `ลบทั้งหมด (${filteredIngredients.length})` : `Delete All (${filteredIngredients.length})`) : (th ? `ปิดการขายทั้งหมด (${filteredProducts.length})` : `Deactivate All (${filteredProducts.length})`)}
                </button>
              </div>
              <button type="button" onClick={() => { setProductSearchText(""); setProductCategoryFilter("all"); setIngredientSearchText(""); setIngredientUnitFilter("all"); setCurrentPage(1); }} className="inline-flex min-h-9 items-center rounded-lg border border-slate-300 bg-white px-3 text-xs font-bold text-slate-700 hover:bg-slate-50">{th ? "ล้างตัวกรอง" : "Clear Filters"}</button>
            </div>
          </div>
        </div>
      ) : null}
      ...
      {noticeOpen ? (
        <div className="fixed inset-0 z-[170] grid place-items-center bg-slate-900/35 p-4" onClick={closeNoticePopup}>
          <div className="w-full max-w-md rounded-2xl border border-sky-200 bg-white p-5 shadow-2xl" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="mt-4 flex justify-end"><button type="button" onClick={closeNoticePopup} className="inline-flex min-h-10 items-center rounded-lg border border-sky-600 bg-sky-600 px-4 text-sm font-bold text-white hover:bg-sky-700">{th ? "ตกลง" : "OK"}</button></div>
          </div>
        </div>
      ) : null}
      ...
      {modeFilter === "ingredients" ? (
          ...
          <table className="min-w-[820px] w-full border-collapse">
            <thead className="sticky top-0 z-10 bg-slate-50">
              <tr className="bg-slate-50">
                <th className="border-b border-slate-200 px-3 py-2 text-left"><input type="checkbox" checked={filteredIngredients.length > 0 && selectedFilteredIngredientIds.length === filteredIngredients.length} onChange={toggleSelectAllFilteredIngredients} aria-label={th ? "เลือกวัตถุดิบทั้งหมดตามตัวกรอง" : "Select all filtered ingredients"} className="h-4 w-4 rounded border-slate-300" /></th>
            ...
            <tbody>
              {filteredIngredients.length === 0 ? <tr><td colSpan={6} className="px-3 py-6 text-center text-sm text-slate-500">{th ? "ไม่พบรายการวัตถุดิบตามตัวกรองนี้" : "No ingredients match this filter."}</td></tr> : pagedIngredients.map((item) => (
                <tr key={item.id}>
                  <td className="border-b border-slate-100 px-3 py-3"><input type="checkbox" checked={selectedIngredientIds.includes(item.id)} onChange={() => toggleIngredientSelection(item.id)} aria-label={th ? `เลือกรายการวัตถุดิบ ${item.name}` : `Select ingredient ${item.name}`} className="h-4 w-4 rounded border-slate-300" /></td>
                  ...
                  <td className="border-b border-slate-100 px-3 py-3">{(() => { const status = getIngredientStatus(item); return <span className={`inline-flex min-h-7 items-center rounded-full border px-2.5 text-xs font-bold ${status.className}`}>{status.label}</span>; })()}</td>
                  <td className="border-b border-slate-100 px-3 py-3"><div className="flex flex-wrap items-center gap-1.5">
                    <button type="button" onClick={() => openEditIngredientPopup(item)} disabled={busyIngredientId === item.id} title={th ? "แก้ไขวัตถุดิบ" : "Edit ingredient"} aria-label={th ? "แก้ไขวัตถุดิบ" : "Edit ingredient"} className="inline-flex min-h-8 items-center justify-center rounded-lg border border-slate-200 px-2 py-1 text-slate-700 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden><path d="M12 20h9" /><path d="M16.5 3.5a2.1 2.1 0 0 1 3 3L7 19l-4 1 1-4 12.5-12.5z" /></svg></button>
                    <button type="button" onClick={() => openDeleteIngredientPopup(item)} disabled={busyIngredientId === item.id} title={th ? "ลบวัตถุดิบ" : "Delete ingredient"} aria-label={th ? "ลบวัตถุดิบ" : "Delete ingredient"} className="inline-flex min-h-8 items-center justify-center rounded-lg border border-red-200 px-2 py-1 text-red-700 hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-60"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden><path d="M3 6h18" /><path d="M8 6V4h8v2" /><path d="M19 6l-1 14H6L5 6" /><path d="M10 11v6" /><path d="M14 11v6" /></svg></button>
                    <button type="button" onClick={() => openIngredientStockPopup(item)} disabled={busyIngredientId === item.id} title={th ? "ปรับสต๊อกวัตถุดิบ" : "Adjust ingredient stock"} aria-label={th ? "ปรับสต๊อกวัตถุดิบ" : "Adjust ingredient stock"} className="inline-flex min-h-8 items-center justify-center rounded-lg border border-emerald-200 px-2 py-1 text-emerald-700 hover:bg-emerald-50 disabled:cursor-not-allowed disabled:opacity-60"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden><path d="M12 5v14" /><path d="M5 12h14" /></svg></button>
                  </div></td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="max-h-[56vh] overflow-auto rounded-xl border border-slate-200">
          <table className="min-w-[1040px] w-full border-collapse">
            <thead className="sticky top-0 z-10 bg-slate-50">
              <tr className="bg-slate-50">
                <th className="border-b border-slate-200 px-3 py-2 text-left"><input type="checkbox" checked={filteredProducts.length > 0 && selectedFilteredProductIds.length === filteredProducts.length} onChange={toggleSelectAllFilteredProducts} aria-label={th ? "เลือกสินค้าทั้งหมดตามตัวกรอง" : "Select all filtered products"} className="h-4 w-4 rounded border-slate-300" /></th>
            ...
            <tbody>
              {filteredProducts.length === 0 ? <tr><td colSpan={9} className="px-3 py-6 text-center text-sm text-slate-500">{th ? "ไม่พบรายการสินค้าตามตัวกรองนี้" : "No products match this filter."}</td></tr> : pagedProducts.map((item) => (
                <tr key={item.id}>
                  <td className="border-b border-slate-100 px-3 py-3"><input type="checkbox" checked={selectedProductIds.includes(item.id)} onChange={() => toggleProductSelection(item.id)} aria-label={th ? `เลือกรายการสินค้า ${item.name}` : `Select product ${item.name}`} className="h-4 w-4 rounded border-slate-300" /></td>
                  ...
                  <td className="border-b border-slate-100 px-3 py-3"><p className="text-sm font-bold text-slate-900">{item.name}</p></td>
                  <td className="border-b border-slate-100 px-3 py-3">{item.stock_on_hand_units === null ? <div className="flex items-center gap-1.5"><p className="text-sm font-bold text-slate-600">-</p>{item.has_ingredient_recipe ? <span className="inline-flex h-4 w-4 items-center justify-center rounded-full border border-emerald-300 bg-emerald-50 text-[10px] font-extrabold leading-none text-emerald-700">+</span> : null}</div> : <div className="flex items-center gap-1.5"><p className="text-sm font-bold text-slate-900">{item.stock_on_hand_units}</p>{item.has_ingredient_recipe ? <span className="inline-flex h-4 w-4 items-center justify-center rounded-full border border-emerald-300 bg-emerald-50 text-[10px] font-extrabold leading-none text-emerald-700">+</span> : null}</div>}</td>
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <strong className="min-w-20 text-center text-xs font-bold text-slate-600">{th ? `หน้า ${currentPage} / ${totalPages}` : `Page ${currentPage} / ${totalPages}`}</strong>
          <button type="button" onClick={() => setCurrentPage((prev) => Math.min(totalPages, prev + 1))} disabled={currentPage >= totalPages} className="inline-flex min-h-8 items-center rounded-lg border border-slate-300 bg-white px-3 text-xs font-semibold text-slate-700 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50">{th ? "ถัดไป" : "Next"}</button>
        </div>
      </div>

      {stockPopupProduct ? (
        <div className="fixed inset-0 z-[150] grid place-items-center bg-slate-900/55 p-4" onClick={closeStockPopup}>
          <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-4 shadow-2xl" onClick={(event) => event.stopPropagation()}>
            <div className="mb-3"><h4 className="text-base font-extrabold text-slate-900">{th ? `ปรับสต๊อก: ${stockPopupProduct.name}` : `Adjust Stock: ${stockPopupProduct.name}`}</h4><p className="mt-1 text-xs text-slate-500">{th ? `สต๊อกปัจจุบัน ${Math.max(0, Number(stockPopupProduct.stock_on_hand_units ?? 0))}` : `Current stock: ${Math.max(0, Number(stockPopupProduct.stock_on_hand_units ?? 0))}`}</p></div>
            <div className="mb-3 grid grid-cols-2 gap-2">
              <button type="button" onClick={() => { setStockPopupMode("add"); setStockPopupQty((prev) => (prev.trim() ? prev : "1")); setStockPopupError(""); }} className={`inline-flex min-h-10 items-center justify-center rounded-lg border text-sm font-bold transition ${stockPopupMode === "add" ? "border-emerald-600 bg-emerald-600 text-white" : "border-emerald-200 bg-emerald-50 text-emerald-700 hover:bg-emerald-100"}`}><span className="inline-flex flex-col items-center leading-none"><span className="text-base font-extrabold">+</span><span>{th ? "เพิ่มสต๊อก" : "Add Stock"}</span></span></button>
              <button type="button" onClick={() => { setStockPopupMode("subtract"); setStockPopupQty((prev) => (prev.trim() ? prev : "1")); setStockPopupError(""); }} className={`inline-flex min-h-10 items-center justify-center rounded-lg border text-sm font-bold transition ${stockPopupMode === "subtract" ? "border-red-600 bg-red-600 text-white" : "border-red-200 bg-red-50 text-red-700 hover:bg-red-100"}`}><span className="inline-flex flex-col items-center leading-none"><span className="text-base font-extrabold">-</span><span>{th ? "ตัดสต๊อก" : "Subtract Stock"}</span></span></button>
            </div>
            {stockPopupMode ? <label className="grid gap-1 text-xs font-semibold text-slate-700"><span>{th ? "จำนวน" : "Quantity"}</span><input value={stockPopupQty} onChange={(event) => setStockPopupQty(event.target.value)} type="number" min={1} step="1" className="min-h-10 rounded-lg border border-slate-300 bg-white px-3 text-sm text-slate-900" /></label> : <p className="text-xs font-semibold text-slate-500">{th ? "เลือกโหมดก่อน แล้วช่องใส่จำนวนจะปรากฏ" : "Select a mode first, then quantity input will appear."}</p>}
            {stockPopupError ? <p className="mt-3 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs font-semibold text-red-700">{stockPopupError}</p> : null}
            <div className="mt-4 flex justify-end gap-2"><button type="button" onClick={closeStockPopup} className="inline-flex min-h-10 items-center rounded-lg border border-slate-300 px-4 text-sm font-semibold text-slate-700 hover:bg-slate-50">{th ? "ยกเลิก" : "Cancel"}</button><button type="button" onClick={() => void submitStockPopup()} disabled={busyProductId === stockPopupProduct.id} className="inline-flex min-h-10 items-center rounded-lg border border-blue-600 bg-blue-600 px-4 text-sm font-bold text-white hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-60">{busyProductId === stockPopupProduct.id ? (th ? "กำลังบันทึก..." : "Saving...") : th ? "บันทึก" : "Save"}</button></div>
          </div>
        </div>
      ) : null}
        ...
        <div className="fixed inset-0 z-[150] grid place-items-center bg-slate-900/55 p-4" onClick={closeBulkDeletePopup}>
          <div className="w-full max-w-lg rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl" onClick={(event) => event.stopPropagation()}>
            <div className="mb-4 flex items-start gap-3">
              <div className={`inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-full border ${bulkDeletePopupMode === "ingredients" ? "border-red-200 bg-red-50 text-red-600" : "border-amber-200 bg-amber-50 text-amber-700"}`}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden><path d="M3 6h18" /><path d="M8 6V4h8v2" /><path d="M19 6l-1 14H6L5 6" /><path d="M10 11v6" /><path d="M14 11v6" /></svg></div>
              <div><h4 className="text-base font-extrabold text-slate-900">{bulkDeletePopupMode === "ingredients" ? (th ? (bulkDeleteScope === "selected" ? "ยืนยันลบวัตถุดิบที่เลือก" : "ยืนยันลบวัตถุดิบทั้งหมด") : (bulkDeleteScope === "selected" ? "Confirm Delete Selected Ingredients" : "Confirm Delete All Ingredients")) : (th ? (bulkDeleteScope === "selected" ? "ยืนยันปิดการขายสินค้าที่เลือก" : "ยืนยันปิดการขายสินค้าทั้งหมด") : (bulkDeleteScope === "selected" ? "Confirm Deactivate Selected Products" : "Confirm Deactivate All Products"))}</h4>
                <p className="mt-1 text-sm text-slate-600">{bulkDeletePopupMode === "ingredients" ? (th ? (bulkDeleteScope === "selected" ? `ต้องการลบวัตถุดิบที่เลือก ${selectedFilteredIngredientIds.length} รายการ ใช่หรือไม่` : `ต้องการลบวัตถุดิบตามตัวกรองทั้งหมด ${filteredIngredients.length} รายการ ใช่หรือไม่`) : (bulkDeleteScope === "selected" ? `Delete ${selectedFilteredIngredientIds.length} selected ingredients?` : `Delete all ${filteredIngredients.length} filtered ingredients?`)) : (th ? (bulkDeleteScope === "selected" ? `ต้องการปิดการขายสินค้าที่เลือก ${selectedFilteredProductIds.length} รายการ ใช่หรือไม่` : `ต้องการปิดการขายสินค้าตามตัวกรองทั้งหมด ${filteredProducts.length} รายการ ใช่หรือไม่`) : (bulkDeleteScope === "selected" ? `Deactivate ${selectedFilteredProductIds.length} selected products?` : `Deactivate all ${filteredProducts.length} filtered products?`))}</p>
                <p className="mt-1 text-xs text-slate-500">{bulkDeletePopupMode === "ingredients" ? (th ? "รายการที่ยังมีประวัติสต๊อกจะถูก archive อัตโนมัติ" : "Rows with stock history will be archived automatically.") : (th ? "สินค้าจะถูกปิดการขาย แต่ประวัติเดิมยังคงอยู่" : "Products will be hidden from sales while keeping history.")}</p></div>
            </div>
            {bulkDeleteError ? <p className="mb-3 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs font-semibold text-red-700">{bulkDeleteError}</p> : null}
            <div className="flex justify-end gap-2"><button type="button" onClick={closeBulkDeletePopup} disabled={bulkDeleteBusy} className="inline-flex min-h-10 items-center rounded-lg border border-slate-300 px-4 text-sm font-semibold text-slate-700 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60">{th ? "ยกเลิก" : "Cancel"}</button><button type="button" onClick={() => void submitBulkDelete()} disabled={bulkDeleteBusy} className={`inline-flex min-h-10 items-center rounded-lg px-4 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-60 ${bulkDeletePopupMode === "ingredients" ? "border border-red-600 bg-red-600 hover:bg-red-700" : "border border-amber-500 bg-amber-500 hover:bg-amber-600"}`}>{bulkDeleteBusy ? (th ? "กำลังดำเนินการ..." : "Processing...") : bulkDeletePopupMode === "ingredients" ? (th ? (bulkDeleteScope === "selected" ? "ลบที่เลือก" : "ลบทั้งหมด") : (bulkDeleteScope === "selected" ? "Delete Selected" : "Delete All")) : (th ? (bulkDeleteScope === "selected" ? "ปิดการขายที่เลือก" : "ปิดการขายทั้งหมด") : (bulkDeleteScope === "selected" ? "Deactivate Selected" : "Deactivate All"))}</button></div>
          </div>
        </div>
      ) : null}
      ...
      {bulkUnlinkScope ? (
        <div className="fixed inset-0 z-[155] grid place-items-center bg-slate-900/55 p-4" onClick={closeBulkUnlinkPopup}>
          <div className="w-full max-w-lg rounded-2xl border border-sky-200 bg-white p-5 shadow-2xl" onClick={(event) => event.stopPropagation()}>
            ...
            <p className="mt-2 text-sm text-slate-600">{th ? (bulkUnlinkScope === "selected" ? `ระบบจะยกเลิกสูตรวัตถุดิบของสินค้าที่เลือก ${selectedFilteredProductIds.length} รายการ แล้วเปลี่ยนกลับเป็นสต๊อกแบบชิ้น` : `ระบบจะยกเลิกสูตรวัตถุดิบของสินค้าตามตัวกรองทั้งหมด ${filteredProducts.length} รายการ แล้วเปลี่ยนกลับเป็นสต๊อกแบบชิ้น`) : (bulkUnlinkScope === "selected" ? `Unlink ingredient recipes for ${selectedFilteredProductIds.length} selected products and switch them back to unit stock.` : `Unlink ingredient recipes for all ${filteredProducts.length} filtered products and switch them back to unit stock.`)}</p>
            <p className="mt-2 rounded-lg border border-sky-100 bg-sky-50 px-3 py-2 text-xs font-semibold text-sky-800">{th ? "ระบบจะไม่ลบประวัติการขาย ใบเสร็จ หรือรายการปิดกะเดิม" : "Sales history, receipts, and shift reports will not be deleted."}</p>
            ...
            <div className="mt-4 flex justify-end gap-2"><button type="button" onClick={closeBulkUnlinkPopup} disabled={bulkUnlinkBusy} className="inline-flex min-h-10 items-center rounded-lg border border-slate-300 px-4 text-sm font-semibold text-slate-700 hover:bg-slate-50 disabled:opacity-60">{th ? "ยกเลิก" : "Cancel"}</button><button type="button" onClick={() => void submitBulkUnlinkRecipes()} disabled={bulkUnlinkBusy} className="inline-flex min-h-10 items-center rounded-lg border border-sky-600 bg-sky-600 px-4 text-sm font-bold text-white hover:bg-sky-700 disabled:opacity-60">{bulkUnlinkBusy ? (th ? "กำลังบันทึก..." : "Saving...") : th ? "บันทึกการเปลี่ยนแปลง" : "Save Changes"}</button></div>
          </div>
        </div>
      ) : null}
      ...
      {deactivatingProduct ? (
        <div className="fixed inset-0 z-[150] grid place-items-center bg-slate-900/55 p-4" onClick={closeDeactivateProductPopup}>
          <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl" onClick={(event) => event.stopPropagation()}>
            <div className="mb-4 flex items-start gap-3"><div className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-amber-200 bg-amber-50 text-amber-700"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden><circle cx="12" cy="12" r="10" /><path d="M12 8v5" /><path d="M12 16h.01" /></svg></div><div><h4 className="text-base font-extrabold text-slate-900">{th ? "ยืนยันปิดการขายสินค้า" : "Confirm Product Deactivation"}</h4><p className="mt-1 text-sm text-slate-600">{th ? `คุณต้องการปิดการขาย "${deactivatingProduct.name}" ใช่หรือไม่` : `Do you want to deactivate "${deactivatingProduct.name}"?`}</p><p className="mt-1 text-xs text-slate-500">{th ? "สินค้าจะไม่แสดงในรายการขาย แต่ประวัติเดิมยังคงอยู่" : "This hides the product from sales while keeping historical records."}</p></div></div>
            {deactivatingProductError ? <p className="mb-3 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs font-semibold text-red-700">{deactivatingProductError}</p> : null}
            <div className="flex justify-end gap-2"><button type="button" onClick={closeDeactivateProductPopup} disabled={busyProductId === deactivatingProduct.id} className="inline-flex min-h-10 items-center rounded-lg border border-slate-300 px-4 text-sm font-semibold text-slate-700 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60">{th ? "ยกเลิก" : "Cancel"}</button><button type="button" onClick={() => void submitDeactivateProduct()} disabled={busyProductId === deactivatingProduct.id} className="inline-flex min-h-10 items-center rounded-lg border border-amber-500 bg-amber-500 px-4 text-sm font-bold text-white hover:bg-amber-600 disabled:cursor-not-allowed disabled:opacity-60">{busyProductId === deactivatingProduct.id ? (th ? "กำลังบันทึก..." : "Saving...") : th ? "ปิดการขาย" : "Deactivate"}</button></div>
          </div>
        </div>
      ) : null}
      ...
      {editingIngredient ? (
        <div className="fixed inset-0 z-[150] grid place-items-center bg-slate-900/55 p-4" onClick={closeEditIngredientPopup}>
          <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-4 shadow-2xl" onClick={(event) => event.stopPropagation()}>
            <div className="mb-3"><h4 className="text-base font-extrabold text-slate-900">{th ? `แก้ไขวัตถุดิบ: ${editingIngredient.name}` : `Edit Ingredient: ${editingIngredient.name}`}</h4></div>
            <div className="grid gap-3">
              <label className="grid gap-1 text-xs font-semibold text-slate-700"><span>{th ? "ชื่อวัตถุดิบ" : "Ingredient Name"}</span><input value={editingIngredientName} onChange={(event) => setEditingIngredientName(event.target.value)} type="text" className="min-h-10 rounded-lg border border-slate-300 bg-white px-3 text-sm text-slate-900" /></label>
              <label className="grid gap-1 text-xs font-semibold text-slate-700"><span>{th ? "จำนวนคงเหลือ" : "Quantity on Hand"}</span><input value={editingIngredientQty} onChange={(event) => setEditingIngredientQty(event.target.value)} type="number" min={0} step="0.01" className="min-h-10 rounded-lg border border-slate-300 bg-white px-3 text-sm text-slate-900" /></label>
              <label className="grid gap-1 text-xs font-semibold text-slate-700"><span>{th ? "หน่วย" : "Unit"}</span><select value={editingIngredientUnit} onChange={(event) => setEditingIngredientUnit(event.target.value)} className="min-h-10 rounded-lg border border-slate-300 bg-white px-3 text-sm text-slate-900">{INGREDIENT_BASE_UNIT_OPTIONS.map((option) => <option key={option.value} value={option.value}>{th ? option.thLabel : option.enLabel}</option>)}{!INGREDIENT_BASE_UNIT_OPTIONS.some((option) => option.value === editingIngredientUnit) && editingIngredientUnit ? <option value={editingIngredientUnit}>{th ? `ค่าเดิม: ${editingIngredientUnit}` : `Current: ${editingIngredientUnit}`}</option> : null}</select></label>
            </div>
            {editingIngredientError ? <p className="mt-3 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs font-semibold text-red-700">{editingIngredientError}</p> : null}
            <div className="mt-4 flex justify-end gap-2"><button type="button" onClick={closeEditIngredientPopup} className="inline-flex min-h-10 items-center rounded-lg border border-slate-300 px-4 text-sm font-semibold text-slate-700 hover:bg-slate-50">{th ? "ยกเลิก" : "Cancel"}</button><button type="button" onClick={() => void submitEditIngredient()} disabled={busyIngredientId === editingIngredient.id} className="inline-flex min-h-10 items-center rounded-lg border border-blue-600 bg-blue-600 px-4 text-sm font-bold text-white hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-60">{busyIngredientId === editingIngredient.id ? (th ? "กำลังบันทึก..." : "Saving...") : th ? "บันทึก" : "Save"}</button></div>
          </div>
        </div>
      ) : null}
      ...
      {deletingIngredient ? (
        <div className="fixed inset-0 z-[150] grid place-items-center bg-slate-900/55 p-4" onClick={closeDeleteIngredientPopup}>
          <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl" onClick={(event) => event.stopPropagation()}>
            <div className="mb-4 flex items-start gap-3"><div className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-red-200 bg-red-50 text-red-600"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden><path d="M3 6h18" /><path d="M8 6V4h8v2" /><path d="M19 6l-1 14H6L5 6" /><path d="M10 11v6" /><path d="M14 11v6" /></svg></div><div><h4 className="text-base font-extrabold text-slate-900">{th ? "ยืนยันการลบวัตถุดิบ" : "Confirm Ingredient Delete"}</h4><p className="mt-1 text-sm text-slate-600">{th ? `คุณต้องการลบ "${deletingIngredient.name}" ใช่หรือไม่` : `Do you want to delete "${deletingIngredient.name}"?`}</p><p className="mt-1 text-xs text-slate-500">{th ? "หากวัตถุดิบนี้ถูกใช้งานในข้อมูลอื่น ระบบอาจไม่อนุญาตให้ลบ" : "Delete may be blocked if this ingredient is still referenced."}</p></div></div>
            {deletingIngredientError ? <p className="mb-3 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs font-semibold text-red-700">{deletingIngredientError}</p> : null}
            <div className="flex justify-end gap-2"><button type="button" onClick={closeDeleteIngredientPopup} disabled={busyIngredientId === deletingIngredient.id} className="inline-flex min-h-10 items-center rounded-lg border border-slate-300 px-4 text-sm font-semibold text-slate-700 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60">{th ? "ยกเลิก" : "Cancel"}</button><button type="button" onClick={() => void submitDeleteIngredient()} disabled={busyIngredientId === deletingIngredient.id} className="inline-flex min-h-10 items-center rounded-lg border border-red-600 bg-red-600 px-4 text-sm font-bold text-white hover:bg-red-700 disabled:cursor-not-allowed disabled:opacity-60">{busyIngredientId === deletingIngredient.id ? (th ? "กำลังลบ..." : "Deleting...") : th ? "ลบวัตถุดิบ" : "Delete"}</button></div>
          </div>
        </div>
      ) : null}
      ...
      {ingredientStockPopup ? (
        <div className="fixed inset-0 z-[150] grid place-items-center bg-slate-900/55 p-4" onClick={closeIngredientStockPopup}>
          <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-4 shadow-2xl" onClick={(event) => event.stopPropagation()}>
            <div className="mb-3"><h4 className="text-base font-extrabold text-slate-900">{th ? `ปรับสต๊อก: ${ingredientStockPopup.name}` : `Adjust Stock: ${ingredientStockPopup.name}`}</h4><p className="mt-1 text-xs text-slate-500">{th ? `คงเหลือปัจจุบัน ${formatIngredientQty(ingredientStockPopup)} ${ingredientStockPopup.baseUnit}` : `Current stock: ${formatIngredientQty(ingredientStockPopup)} ${ingredientStockPopup.baseUnit}`}</p></div>
            <div className="mb-3 grid grid-cols-2 gap-2">
              <button type="button" onClick={() => { setIngredientStockPopupMode("add"); setIngredientStockPopupQty((prev) => (prev.trim() ? prev : "1")); setIngredientStockPopupError(""); }} className={`inline-flex min-h-10 items-center justify-center rounded-lg border text-sm font-bold transition ${ingredientStockPopupMode === "add" ? "border-emerald-600 bg-emerald-600 text-white" : "border-emerald-200 bg-emerald-50 text-emerald-700 hover:bg-emerald-100"}`}><span className="inline-flex flex-col items-center leading-none"><span className="text-base font-extrabold">+</span><span>{th ? "เพิ่มสต๊อก" : "Add Stock"}</span></span></button>
              <button type="button" onClick={() => { setIngredientStockPopupMode("subtract"); setIngredientStockPopupQty((prev) => (prev.trim() ? prev : "1")); setIngredientStockPopupError(""); }} className={`inline-flex min-h-10 items-center justify-center rounded-lg border text-sm font-bold transition ${ingredientStockPopupMode === "subtract" ? "border-red-600 bg-red-600 text-white" : "border-red-200 bg-red-50 text-red-700 hover:bg-red-100"}`}><span className="inline-flex flex-col items-center leading-none"><span className="text-base font-extrabold">-</span><span>{th ? "ตัดสต๊อก" : "Subtract Stock"}</span></span></button>
            </div>
            {ingredientStockPopupMode ? <label className="grid gap-1 text-xs font-semibold text-slate-700"><span>{th ? "จำนวน" : "Quantity"}</span><input value={ingredientStockPopupQty} onChange={(event) => setIngredientStockPopupQty(event.target.value)} type="number" min={1} step="1" className="min-h-10 rounded-lg border border-slate-300 bg-white px-3 text-sm text-slate-900" /></label> : <p className="text-xs font-semibold text-slate-500">{th ? "เลือกโหมดก่อน แล้วช่องใส่จำนวนจะปรากฏ" : "Select a mode first, then quantity input will appear."}</p>}
            {ingredientStockPopupError ? <p className="mt-3 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs font-semibold text-red-700">{ingredientStockPopupError}</p> : null}
            <div className="mt-4 flex justify-end gap-2"><button type="button" onClick={closeIngredientStockPopup} className="inline-flex min-h-10 items-center rounded-lg border border-slate-300 px-4 text-sm font-semibold text-slate-700 hover:bg-slate-50">{th ? "ยกเลิก" : "Cancel"}</button><button type="button" onClick={() => void submitIngredientStockPopup()} disabled={busyIngredientId === ingredientStockPopup.id} className="inline-flex min-h-10 items-center rounded-lg border border-blue-600 bg-blue-600 px-4 text-sm font-bold text-white hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-60">{busyIngredientId === ingredientStockPopup.id ? (th ? "กำลังบันทึก..." : "Saving...") : th ? "บันทึก" : "Save"}</button></div>
          </div>
        </div>
      ) : null}
    </div>
