...
export function PosSalesModule({ lang = "th" }: { lang?: Lang }) {
    ...
    const pollTableQrOrders = async () => {
      ...
      try {
        ...
        const unseen = (body.data?.items ?? []).filter((entry) => {
          ...
          const isOrderEvent = !entry.event_type || entry.event_type === "order";
          const reviewStatus = String(entry.review_status ?? entry.payload?.review_status ?? "");
          if (
            isOrderEvent &&
            (isSuppressedTableQrReviewId(entry.id) || (reviewStatus.length > 0 && reviewStatus !== "pending_pos_review"))
          ) {
  ...
  async function submitTableQrReview(action: "accept" | "reject") {
    ...
    if (review.busy || isSuppressedTableQrReviewId(review.submissionId)) {
      if (tableQrReviewTerminalRef.current.has(review.submissionId)) setTableQrReview(null);
      return;
    }
    ...
    try {
      const { response, body } = await fetchJsonWithTimeout<ApiErrorBody>(`/api/pos/tables/${review.tableId}/qr-orders`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
