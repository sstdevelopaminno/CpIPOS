...
const DESKTOP_RELEASE_TAG = "v0.3.3";
const DESKTOP_VERSION = DESKTOP_RELEASE_TAG.replace(/^v/, "");
const DESKTOP_REPO = "sstdevelopaminno/cp-ipos-desktop";

const releaseApiUrl = `https://api.github.com/repos/${DESKTOP_REPO}/releases/tags/${DESKTOP_RELEASE_TAG}`;
const expectedAssetName = "CpIPOS.Desktop_0.3.3_x64-setup.exe";
  ...
  try {
    const response = await fetch(releaseApiUrl, {
      ...
      headers: {
        ...
        "User-Agent": "CpIPOS-Windows-Download"
      }
    });

    if (!response.ok) {
      return notReady(`ยังไม่พบ Release ${DESKTOP_RELEASE_TAG} สำหรับ Windows`);
    }
    ...
    if (!asset?.browser_download_url) {
      return notReady(`พบ Release ${DESKTOP_RELEASE_TAG} แล้ว แต่ยังไม่พบไฟล์ ${expectedAssetName}`);
    }

    const redirect = NextResponse.redirect(asset.browser_download_url, 302);
    redirect.headers.set("X-CpIPOS-Windows-Version", DESKTOP_VERSION);
    redirect.headers.set("Cache-Control", "no-store");
    return redirect;
  } catch {
    return notReady(`ยังตรวจสอบไฟล์ Windows ${DESKTOP_VERSION} ไม่ได้ กรุณาลองใหม่อีกครั้ง`);
  }
}
...
<head>
  ...
  <title>CpIPOS Windows ${DESKTOP_VERSION}</title>
  <style>
    ...
    main{max-width:720px;border:1px solid #334155;border-radius:24px;background:#0f172a;padding:28px;box-shadow:0 24px 80px rgba(0,0,0,.35)}
...
<body>
  <main>
    <h1>CpIPOS Windows ${DESKTOP_VERSION}</h1>
