...
const apps = [
  {
    ...
    platform: "Android Tablet POS · v1.0.8",
    description: "CpIPOS Android POS 1.0.8 Stable สำหรับเครื่องขายหน้าร้าน พร้อม deterministic multi-USB printer binding, Native Print Agent, MDM และการปรับขนาด launcher icon ให้สมดุลขึ้น",
    file: "CpIPOS-Android-POS-1.0.8.apk",
    status: "1.0.8 Stable · พร้อมดาวน์โหลด",
    ready: true,
    href: ANDROID_POS_108_URL
  },
  {
...
export function DownloadCenterV108() {
  return (
    <main className="min-h-dvh overflow-y-auto bg-[radial-gradient(circle_at_top,_#0b2447_0%,_#061227_34%,_#020617_72%)] text-slate-100">
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8">
        ...
        <section className="mx-auto mt-16 max-w-4xl text-center sm:mt-20">
          ...
          <p className="mx-auto mt-5 max-w-2xl text-base leading-7 text-slate-300 sm:text-lg">ดาวน์โหลด CpIPOS POS - Android Tablet เวอร์ชัน 1.0.8 Stable สำหรับเครื่องขายหน้าร้านได้จากหน้านี้โดยตรง</p>
          <div className="mt-7 flex flex-wrap items-center justify-center gap-3 text-xs font-semibold">
            <span className="rounded-full border border-emerald-400/20 bg-emerald-400/10 px-3 py-1.5 text-emerald-300">Android POS 1.0.8 Stable พร้อมดาวน์โหลด</span>
            ...
            <article key={app.title} className={`relative flex min-h-[390px] flex-col overflow-hidden rounded-3xl border p-6 shadow-2xl sm:p-7 ${app.ready ? "border-sky-400/35 bg-gradient-to-b from-sky-950/70 to-slate-900/90 shadow-sky-950/30" : "border-slate-800 bg-slate-900/75 shadow-black/20"}`}>
              ...
              <div className="mt-auto pt-7">
                ...
                {app.ready && "href" in app ? (
                  <a href={app.href} className="inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-sky-400 px-5 py-3 text-sm font-black text-slate-950 shadow-lg shadow-sky-950/40 transition hover:bg-sky-300">ดาวน์โหลด Android POS 1.0.8</a>
                ) : (
        ...
        <section className="mt-8 rounded-2xl border border-slate-800 bg-slate-900/50 px-5 py-4 text-center text-sm text-slate-400">Android POS 1.0.8 สามารถติดตั้งทับเวอร์ชันเดิมที่ใช้ stable signing เดียวกันได้ โดยไม่ต้องถอนแอปก่อน</section>
      </div>
    </main>
  );
}
