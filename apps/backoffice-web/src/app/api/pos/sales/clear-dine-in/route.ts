...
export async function POST(request: Request) {
  try {
    ...
    if (!updated?.order_id) return fail("dine_in_clear_failed", "Dine-in clear returned no order state.", 500);
    ...
    await queueMissingKitchenPrintJobsForOrder({ auth, orderId, runtimeDeviceCode: scope.session.device_code }).catch((error) => {
