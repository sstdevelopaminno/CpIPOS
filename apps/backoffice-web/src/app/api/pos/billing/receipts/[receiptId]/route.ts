...
export async function GET(_request: Request, { params }: Params) {
  try {
    ...
    const annotationResult = await db.from("tenant_subscription_receipt_annotations")
      .select("correction_note,voided_at").eq("receipt_id",receipt.id).eq("tenant_id",scope.session.tenant_id)
