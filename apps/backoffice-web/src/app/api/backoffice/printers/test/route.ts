...
function agentMatchesProfile(
  profile: Awaited<ReturnType<typeof listPrinterProfiles>>[number],
  agent: Awaited<ReturnType<typeof listPrintAgents>>[number]
) {
  const metadata = asRecord(profile.metadata);
...
function isFreshAgent(agent: Awaited<ReturnType<typeof listPrintAgents>>[number]) {
  if (agent.status !== "active" || !agent.last_seen_at) return false;
  const seenAt = new Date(agent.last_seen_at).getTime();
  return Number.isFinite(seenAt) && Date.now() - seenAt <= PRINT_AGENT_FRESH_MS;
}

async function assertPrinterReadyForTest(
  auth: Awaited<ReturnType<typeof getAuthContext>>,
  printerId: string
) {
  const profiles = await listPrinterProfiles(auth);
  const profile = profiles.find((item) => item.id === printerId);
  if (!profile) throw new Error("printer_not_found");
  if (!profile.enabled) throw new Error("printer_disabled");

  if (profile.connection_type === "NETWORK_ESC_POS" && !profile.ip_address?.trim()) {
    throw new Error("printer_network_not_configured");
  }

  if (!requiresPrintAgent(profile)) return profile;

  const agents = await listPrintAgents(auth);
  const compatibleAgents = agents.filter((agent) => agentMatchesProfile(profile, agent));
  if (compatibleAgents.length === 0) throw new Error("printer_agent_not_configured");
  if (!compatibleAgents.some(isFreshAgent)) throw new Error("printer_agent_offline");
  return profile;
}
  ...
  try {
    ...
    const message =
      result?.status === "printed"
        ? "พิมพ์ทดสอบสำเร็จแล้ว"
        : "ส่งงานพิมพ์ทดสอบแล้ว Print Agent ออนไลน์และกำลังรับงาน";
    ...
    if (message === "printer_not_found") {
      return fail("printer_not_found", "ไม่พบเครื่องพิมพ์นี้ในสาขาปัจจุบัน", 404);
    }
    if (message === "printer_disabled") {
      return fail("printer_disabled", "เครื่องพิมพ์นี้ถูกปิดใช้งาน กรุณาเชื่อมต่อใหม่ก่อนทดสอบ", 409);
    }
    if (message === "printer_network_not_configured") {
      return fail("printer_not_configured", "เครื่องพิมพ์ LAN ยังไม่ได้ตั้งค่า IP กรุณาแก้ไข IP/Port ก่อนทดสอบ", 409);
    }
    if (message === "printer_agent_not_configured") {
      return fail(
        "printer_agent_not_configured",
        "ยังไม่มี Print Agent ที่ตรงกับเครื่อง POS/เครื่องพิมพ์นี้ กรุณาเชื่อมต่อ Runtime หรือ Android Print Agent ก่อนทดสอบ",
        503
      );
    }
    if (message === "printer_agent_offline") {
      return fail(
        "printer_agent_offline",
        "Print Agent ของเครื่องนี้ออฟไลน์หรือไม่ได้ heartbeat ภายใน 5 นาที กรุณาเปิด/เชื่อมต่อ Print Agent แล้วลองใหม่",
        503
      );
    }
    if (message.includes("timeout")) {
      return fail("print_agent_unavailable", "Print agent or printer did not respond in time.", 504);
    }
