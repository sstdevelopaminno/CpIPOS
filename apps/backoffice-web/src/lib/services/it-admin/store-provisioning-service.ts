...
function normalizeCoreResult(value: unknown): CoreProvisioningResult {
  ...
  if (!result.tenant?.id || !result.branch?.id || !result.store_code || !result.package?.id) {
    throw new StoreProvisioningError("store_core_incomplete_result", "Store provisioning did not return the required control-plane identifiers.", 500);
  }
  return result;
}
...
async function findAuthUserByEmail(context: ItAdminContext, email: string) {
  ...
  throw new StoreProvisioningError(
    "owner_auth_lookup_limit_reached",
    "Owner identity lookup exceeded the safe pagination limit; resolve the account before provisioning.",
    409
  );
}

async function resolveOrCreateOwnerIdentity(
  context: ItAdminContext,
  input: { email: string; fullName: string; pin: string }
) {
  ...
  if (profileLookupError) {
    throw new StoreProvisioningError("owner_profile_lookup_failed", "Unable to resolve Owner profile.", 500);
  }
  if ((profiles ?? []).length > 1) {
    throw new StoreProvisioningError("owner_email_ambiguous", "More than one Owner profile uses this email; resolve the duplicate identity first.", 409);
  }
  ...
  let authUser = profile ? await context.supabase.auth.admin.getUserById(profile.id) : null;
  ...
  if (!profile) {
    let existingAuthUser = await findAuthUserByEmail(context, input.email);
    if (!existingAuthUser) {
      ...
      const created = await supabase.auth.admin.createUser({
        ...
        user_metadata: {
          ...
          source: "it_store_provisioning_p0"
        }
      });
      if (created.error || !created.data.user) {
        existingAuthUser = await findAuthUserByEmail(context, input.email).catch(() => null);
        if (!existingAuthUser) {
          throw new StoreProvisioningError("owner_auth_create_failed", "Unable to create Owner authentication identity.", 500);
        }
      } else {
        existingAuthUser = created.data.user;
      }
    }
    ...
    authUser = { data: { user: existingAuthUser }, error: null };
  }

  if (!profile) {
    throw new StoreProvisioningError("owner_profile_missing", "Owner profile could not be resolved.", 500);
  }
  ...
  if (profile.pin_hash) {
    ...
    if (!pinMatches) {
      throw new StoreProvisioningError(
        ...
        "This email already belongs to an Owner identity with another PIN. Use the existing PIN or a different Owner email.",
        409
      );
    }
  } else {
...
async function bindOwnerToStore(
  context: ItAdminContext,
  input: {
    tenantId: string;
    branchId: string;
  ...
  if (!currentPosProfile) {
    const { error: posProfileError } = await supabase.from("pos_user_profiles").insert({
    ...
    if (posProfileError) {
      const status = posProfileError.code === "23505" ? 409 : 500;
      throw new StoreProvisioningError("owner_pos_profile_create_failed", "Unable to create Owner POS employee identity.", status);
    }
  }
...
async function markProvisioningRequest(
  context: ItAdminContext,
  requestId: string,
  ...
  if (error) {
    console.error("[store-provisioning] request ledger update failed", { requestId, error: error.message });
  }
}
...
export async function provisionStore(context: ItAdminContext, input: StoreProvisioningInput) {
  ...
  const employeeCode = normalizeEmployeeCode(input.owner?.employee_code) || "100001";
  ...
  if (!storeName || !branchCode || !branchName || !packageId || !ownerName || !ownerEmail) {
    throw new StoreProvisioningError(
      "invalid_store_provisioning_payload",
      "Store name, package, initial branch and Owner name/email are required."
    );
  }
  ...
  if (!/^\d{1,32}$/.test(employeeCode)) {
    throw new StoreProvisioningError("invalid_owner_employee_code", "Owner employee code must contain digits only.");
  }
  if (!/^\d{4,8}$/.test(pin)) {
    throw new StoreProvisioningError("invalid_owner_pin", "Owner PIN must contain 4 to 8 digits.");
  }

  const internalCode = createInternalTenantCode();
  ...
  if (rpcError) {
    ...
    if (rpcError.code === "23505") {
      throw new StoreProvisioningError("store_unique_conflict", "Store or branch identifier already exists.", 409);
    }
  ...
  try {
    const owner = await resolveOrCreateOwnerIdentity(context, {
      email: ownerEmail,
      fullName: ownerName,
      pin
    });
    await bindOwnerToStore(context, {
    ...
    const result = {
      ...
      activation: {
        status: "ready_for_device_enrollment" as const,
        next_step: "register_device" as const
      }
    };
    ...
    throw new StoreProvisioningError(
      ...
      "Store core is safe, but Owner provisioning failed. Retry using the same request_id.",
      500
    );
  }
}
