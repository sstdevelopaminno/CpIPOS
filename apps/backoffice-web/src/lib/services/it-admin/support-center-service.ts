...
function normalizeSecuritySignals(value: unknown): NonNullable<DeviceMdmHealthInput["security_signals"]> {
  if (!Array.isArray(value)) return [];
  const rows: Array<NonNullable<DeviceMdmHealthInput["security_signals"]>[number]> = [];
  for (const item of value) {
    ...
    if (!code || !message || !capturedAt || !severity || !["info", "warning", "critical"].includes(severity)) continue;
    rows.push({
      code,
      message,
