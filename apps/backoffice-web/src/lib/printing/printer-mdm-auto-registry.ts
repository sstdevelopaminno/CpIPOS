...
function extractBluetoothCandidates(inventory: JsonRecord, exactBondedVerification: boolean): AutoCandidate[] {
  ...
  for (const value of rows) {
    ...
    if (String(row.physical_fingerprint_stability ?? "") !== "stable") continue;
    // SPP advertisement alone is never printer identity. Independent printer evidence is
    // required before a bonded device may enter automatic discovery.
    ...
    const verificationSupported = connectPermission && enabled && (sppAdvertised || exactBondedVerification);
    candidates.push({
      fingerprint,
      mode: "bluetooth",
      displayName: name,
      ...
      verificationSupportedNow: verificationSupported,
      capabilities: {
        auto_discovered: true,
        stable_fingerprint: true,
        bonded: true,
        ...
        exact_bonded_verification: exactBondedVerification
      },
      observation: {
        name,
...
function extractCandidates(payload: JsonRecord | null): AutoCandidate[] {
  ...
  const printerCapabilities = asRecord(runtimeCapabilities.printer);
  const exactBondedVerification = printerCapabilities.bluetooth_exact_bonded_verification === true;
  ...
  for (const candidate of [
    ...extractUsbCandidates(inventory),
    ...extractBluetoothCandidates(inventory, exactBondedVerification)
  ]) {
...
async function reconcileCandidates(scope: PairedDeviceScope, candidates: AutoCandidate[]): Promise<PrinterDeviceRow[]> {
  ...
  for (const candidate of candidates) {
    ...
    if (existing && !existing.is_active) {
      // Customer intent wins: a disconnected/deleted physical target is never silently revived.
      continue;
    }
