...
CREATE TABLE IF NOT EXISTS local_license (
  id TEXT PRIMARY KEY,
  license_type TEXT NOT NULL CHECK (license_type IN ('offline_only','cloud_package','test_full_access')),
...
CREATE TABLE IF NOT EXISTS local_entitlements (
  ...
  source TEXT NOT NULL CHECK (source IN ('offline_default','cloud_package','test_full_access','manual_it_override')),
  expires_at TEXT,
  updated_at TEXT NOT NULL DEFAULT (datetime('now'))
);
...
CREATE INDEX IF NOT EXISTS idx_local_orders_tenant_branch ON local_orders(tenant_id, branch_id, created_offline_at);
