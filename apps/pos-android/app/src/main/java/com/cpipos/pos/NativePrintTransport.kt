...
internal class NativePrintTransport(context: Context) {
    ...
    fun print(job: NativePrintJob): NativePrintResult {
        ...
        val result = when {
            ...
            else -> throw NativePrintException("unsupported_transport", false, "Unsupported printer transport: ${job.printer.connectionType}")
        }
        val transportWriteMs = (SystemClock.elapsedRealtime() - transportStartedAt).coerceAtLeast(0L)
        return result.copy(renderMode = prepared.renderMode, payloadBuildMs = payloadBuildMs, transportWriteMs = transportWriteMs)
    }
    ...
    private fun buildPayload(job: NativePrintJob): PreparedPayload {
        ...
        if (command == "open_cash_drawer" || job.payloadText.trim() == "OPEN_CASH_DRAWER") {
            ...
            return PreparedPayload(
                byteArrayOf(0x1B, 0x70, pin.toByte(), (onMs / 2).coerceIn(1, 255).toByte(), (offMs / 2).coerceIn(1, 255).toByte()),
                "command"
            )
        }
    ...
    private fun printLan(printer: NativePrinterProfile, payload: ByteArray): NativePrintResult {
        ...
        try {
            Socket().use { socket ->
                ...
                socket.getOutputStream().use { output -> output.write(payload); output.flush() }
            }
    ...
    private data class UsbTarget(val device: UsbDevice, val usbInterface: UsbInterface, val endpoint: UsbEndpoint)
    ...
    private fun printUsb(printer: NativePrinterProfile, payload: ByteArray): NativePrintResult {
        ...
        try {
            connection = manager.openDevice(target.device) ?: throw NativePrintException("usb_open_failed", true, "Unable to open USB printer")
            if (!connection.claimInterface(target.usbInterface, true)) throw NativePrintException("usb_claim_failed", true, "Unable to claim USB printer interface")
            var offset = 0
            while (offset < payload.size) {
                ...
                if (sent <= 0) throw NativePrintException("usb_write_failed", true, "USB printer did not accept data")
                offset += sent
            }
            return NativePrintResult(payload.size, "usb", "android-usb:${target.device.vendorId}:${target.device.productId}:${target.device.deviceId}")
    ...
    private fun findUsbTarget(printer: NativePrinterProfile): UsbTarget? {
        ...
        val candidates = manager.deviceList.values.asSequence()
            ...
            .filter { requiredDeviceId == null || it.deviceId == requiredDeviceId }
            .mapNotNull { findPrintableUsbTarget(it, allowGenericWritableEndpoint) }
        ...
        candidates.firstOrNull { target -> claimedBindings.none { bindingMatches(manager, it, target.device) } }?.let {
            persistUsbBinding(printer.id, it.device)
            return it
        }
        throw NativePrintException("usb_printer_ambiguous", false, "Multiple USB printer candidates are connected and all physical targets are already bound. Configure usb_device_name, usb_serial_number, usb_vendor_id/product_id, or usb_slot for the intended printer.")
    }
    ...
    private fun findPrintableUsbTarget(device: UsbDevice, allowGenericWritableEndpoint: Boolean): UsbTarget? {
        ...
        val safeAutoSelect = PrinterSelectionPolicy.usbMayAutoSelect(false, manufacturerName, productName)
    ...
    private fun persistUsbBinding(printerId: String, device: UsbDevice) {
        ...
        val binding = if (!serial.isNullOrBlank()) "serial:${device.vendorId}:${device.productId}:$serial" else "path:${device.vendorId}:${device.productId}:${device.deviceName}"
    ...
    private fun bindingMatches(manager: UsbManager, binding: String, device: UsbDevice): Boolean {
        if (binding.startsWith("serial:")) {
            val parts = binding.split(":", limit = 4); if (parts.size != 4) return false
            val vendorId = parts[1].toIntOrNull() ?: return false; val productId = parts[2].toIntOrNull() ?: return false
            return device.vendorId == vendorId && device.productId == productId && safeUsbSerial(manager, device)?.equals(parts[3], ignoreCase = true) == true
        }
        if (binding.startsWith("path:")) {
            val parts = binding.split(":", limit = 4); if (parts.size != 4) return false
            val vendorId = parts[1].toIntOrNull() ?: return false; val productId = parts[2].toIntOrNull() ?: return false
            return device.vendorId == vendorId && device.productId == productId && device.deviceName == parts[3]
        }
        return false
    }
    ...
    private fun printBluetooth(printer: NativePrinterProfile, payload: ByteArray): NativePrintResult {
        ...
        val target = when {
            ...
            preferredName.isNotBlank() -> selectBondedBluetoothByName(bonded, preferredName)
                ...
                val candidates = bonded.filter { PrinterSelectionPolicy.bluetoothMayAutoSelect(runCatching { it.name }.getOrNull()) }
                when (candidates.size) { 0 -> null; 1 -> candidates.first(); else -> throw NativePrintException("bluetooth_printer_ambiguous", false, "Multiple paired Bluetooth printer candidates are available. Configure bluetooth_address or bluetooth_name explicitly.") }
            }
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.S || ContextCompat.checkSelfPermission(appContext, Manifest.permission.BLUETOOTH_SCAN) == PackageManager.PERMISSION_GRANTED) runCatching { adapter.cancelDiscovery() }
        try {
            target.createRfcommSocketToServiceRecord(SPP_UUID).use { socket -> socket.connect(); socket.outputStream.use { it.write(payload); it.flush() } }
    ...
    private fun selectBondedBluetoothByName(bonded: List<android.bluetooth.BluetoothDevice>, preferredName: String): android.bluetooth.BluetoothDevice? {
        val exact = bonded.filter { runCatching { it.name?.equals(preferredName, ignoreCase = true) == true }.getOrDefault(false) }
        if (exact.size == 1) return exact.first()
        if (exact.size > 1) throw NativePrintException("bluetooth_printer_ambiguous", false, "Multiple paired Bluetooth devices have the configured printer name. Configure bluetooth_address explicitly.")
        val partial = bonded.filter { runCatching { it.name?.contains(preferredName, ignoreCase = true) == true }.getOrDefault(false) }
        return when (partial.size) { 0 -> null; 1 -> partial.first(); else -> throw NativePrintException("bluetooth_printer_ambiguous", false, "Multiple paired Bluetooth devices match the configured printer name. Configure bluetooth_address explicitly.") }
    }
