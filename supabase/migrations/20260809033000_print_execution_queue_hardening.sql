-- Printer Execution Queue hardening for CpiPOS-001.
-- Makes claim/lease/ack/fail atomic and records every physical print attempt.
...
create unique index if not exists ux_print_jobs_scope_id
...
create index if not exists idx_print_job_attempts_agent
...
begin
  ...
  -- Reap abandoned leases before issuing new work. An expired physical attempt is
  -- retryable until max_retry_count is reached; no job is ever marked printed here.
  for v_expired in
    select pj.*
  ...
  loop
    ...
    set status = case when v_next_retry < coalesce(v_expired.max_retry_count, 0) then 'retrying'::public.print_job_status else 'failed'::public.print_job_status end,
        ...
        failed_at = case when v_next_retry < coalesce(v_expired.max_retry_count, 0) then null else v_now end,
        updated_at = v_now
  ...
  for v_job in
    ...
    join public.printer_profiles pp
     ...
     and pp.enabled = true
     and pp.printer_role = pj.printer_role
    where pj.tenant_id = p_tenant_id
      and pj.branch_id = p_branch_id
  ...
  loop
    ...
    insert into public.print_job_attempts(
      tenant_id,branch_id,print_job_id,agent_id,agent_attempt_id,attempt_no,status,
      lease_expires_at,claimed_at,metadata
    ) values (
      p_tenant_id,p_branch_id,v_job.id,p_agent_id,v_attempt_id,v_attempt_no,'claimed',
      v_expires,v_now,jsonb_build_object('source','app.claim_print_jobs_v2')
    );
...
begin
  ...
  select * into v_job
  from public.print_jobs pj
  where pj.id = p_job_id and pj.tenant_id = p_tenant_id and pj.branch_id = p_branch_id
  for update;
  if not found then raise exception 'PRINT_JOB_NOT_FOUND'; end if;

  if v_job.status = 'printed'
     and v_job.claimed_by_agent_id = p_agent_id
     and v_job.agent_attempt_id = v_attempt then
    job_id := v_job.id; job_status := v_job.status::text; retry_count := v_job.retry_count; printed_at := v_job.printed_at;
    return next; return;
  end if;
  ...
  set status = 'printed',
      ...
      metadata = coalesce(pj.metadata, '{}'::jsonb)
        ...
        || jsonb_build_object('agent_id',p_agent_id,'agent_attempt_id',v_attempt,'provider_job_id',p_provider_job_id,'bytes_sent',p_bytes_sent),
      updated_at = v_now
  where pj.id = p_job_id and pj.tenant_id = p_tenant_id and pj.branch_id = p_branch_id
...
begin
  ...
  if v_job.status <> 'printing'
     ...
     or v_job.agent_attempt_id is distinct from v_attempt then
    select exists(
      select 1 from public.print_job_attempts a
      where a.tenant_id = p_tenant_id and a.branch_id = p_branch_id
        and a.print_job_id = p_job_id and a.agent_id = p_agent_id
        and a.agent_attempt_id = v_attempt and a.status = 'failed'
  ...
  update public.print_jobs pj
  set status = case when v_can_retry then 'retrying'::public.print_job_status else 'failed'::public.print_job_status end,
      retry_count = v_next_retry,
      claimed_by_agent_id = null,
      claimed_at = null,
      claim_expires_at = null,
...
create or replace function public.claim_print_jobs_v2(
  p_tenant_id uuid,p_branch_id uuid,p_agent_id uuid,p_printer_ids uuid[],p_limit integer default 5,p_lease_seconds integer default 45
)
...
create or replace function public.ack_print_job_v2(
...
create or replace function public.fail_print_job_v2(
  p_tenant_id uuid,p_branch_id uuid,p_job_id uuid,p_agent_id uuid,p_agent_attempt_id text,
