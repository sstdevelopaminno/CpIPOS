-- Kitchen configuration transactions + KDS ticket status transitions.
...
begin
  ...
  if v_scope = 'product' then
    ...
    delete from kitchen_routing_rules r
    where r.tenant_id = p_tenant_id
      and r.branch_id = p_branch_id
      and r.product_id = p_product_id
      and r.category_name is null;

    insert into kitchen_routing_rules(
...
begin
  ...
  if not found then
    raise exception 'KITCHEN_TICKET_NOT_FOUND';
  end if;
  ...
  if v_current in ('ready','cancelled') then
    raise exception 'KITCHEN_STATUS_TERMINAL';
  end if;
