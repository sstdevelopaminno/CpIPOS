-- Prevent newly inserted POS dine-in order items from being enqueued twice.
-- The statement-level order_items INSERT trigger already routes newly inserted rows
-- to the kitchen. replace_queued_dine_in_order_tx must only enqueue positive deltas
-- for order items that existed before this edit.
...
begin
  ...
  -- Only existing order items need an explicit POS edit delta enqueue.
  -- Newly inserted rows were already routed by route_inserted_order_items_to_kitchen.
  update public.order_items oi
  set metadata=coalesce(oi.metadata,'{}'::jsonb)||jsonb_build_object('kitchen_delta_quantity',ti.delta,'kitchen_delta_kind','add')
  from pg_temp.dine_in_target_items ti
  where ti.order_item_id=oi.id and ti.delta>0 and coalesce(ti.existing_quantity,0)>0
    and oi.tenant_id=p_tenant_id and oi.branch_id=p_branch_id and oi.order_id=p_order_id;
  ...
  where order_item_id is not null and delta>0 and coalesce(existing_quantity,0)>0;
