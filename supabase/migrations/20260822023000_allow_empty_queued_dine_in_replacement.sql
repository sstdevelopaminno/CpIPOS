...
-- - This does NOT create empty orders.
-- - The target order must already exist, be queued, be dine-in, and remain bound to the
--   currently open/ordering table bill session.
-- - An explicit empty JSON array means the cashier's desired bill state is empty.
-- - A non-empty payload that normalizes to zero valid items still fails closed.
-- - Removed quantities are retained historically as quantity=0 and are emitted to the
--   kitchen as compensating cancel deltas; kitchen acceptance never owns the bill state.
...
begin
  ...
  -- Empty is a valid desired state ONLY when it was explicitly supplied as an empty
  -- JSON array. A non-empty payload that fails normalization still fails closed.
  if jsonb_array_length(p_items) > 0 and v_item_count < 1 then
    raise exception 'ITEMS_REQUIRED';
  end if;

  -- Bind desired rows to existing active bill lines first. This intentionally permits
  -- a product that was deactivated after it was already sold.
  ...
  -- The cashier owns the bill desired state. Rows not present in that desired state are
  -- retained as zero-quantity history and produce kitchen cancellation deltas.
  update public.order_items oi
  set quantity = 0,
      line_total = 0,
