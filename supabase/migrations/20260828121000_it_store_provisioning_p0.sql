...
-- Atomic core: tenant + immutable Store Code/lifecycle trigger + initial branch + login policy + package contract.
-- Owner Auth/profile provisioning remains server-side because Supabase Auth cannot participate in this DB transaction.
...
begin
  ...
  v_payload := jsonb_build_object(
    'internal_code', upper(btrim(p_internal_code)),
  ...
  insert into public.tenant_subscription_contracts (
    ...
    now(),
    null,
    jsonb_build_object('source', 'it_store_provisioning_p0', 'package_code', v_package.code)
  )
...
revoke all on function public.provision_it_store_core(uuid,uuid,text,text,text,text,text,text,text,text,uuid,text,text) from public, anon, authenticated;
grant execute on function public.provision_it_store_core(uuid,uuid,text,text,text,text,text,text,text,text,uuid,text,text) to service_role;

comment on table public.it_store_provisioning_requests is
  'Idempotency/recovery ledger for IT Store Provisioning. Never stores plaintext Owner PIN.';
comment on function public.provision_it_store_core(uuid,uuid,text,text,text,text,text,text,text,text,uuid,text,text) is
  'Service-role-only atomic Store Provisioning core. Owner Auth/profile is completed by trusted server code.';
