-- FG0004 Primary inactive Restaurant QR provisioning transaction.
-- SOURCE ONLY / DO NOT EXECUTE until explicit production maintenance-window approval.
-- Required order: run read-only collision preflight, close maintenance gate, backup Primary, apply registry/print-guard migration, then run this transaction.
...
begin
  ...
  select p.id into v_growth_package_id
  ...
  select
    ...
    false,
    jsonb_build_object(
      'source', 'fg0004_primary_inactive_restaurant_qr_provisioning',
  ...
  insert into public.branch_devices (
    ...
    is_locked, allow_morning_shift, allow_afternoon_shift, is_active, metadata
  ) values (
    v_pos_device_id,
    v_tenant_id,
    v_branch_id,
  ...
  (
    ...
    null, null, 80, false,
    jsonb_build_object('source', 'fg0004_primary_inactive_restaurant_qr_provisioning', 'slot_only', true, 'hardware_identity', 'TBD', 'automatic_reassignment', false)
  ),
  (
  ...
  where tenant_id = v_tenant_id and branch_id = v_branch_id and table_code between 'T01' and 'T20';

  if v_table_count <> 20 then
    raise exception 'FG0004_TABLE_POSTFLIGHT_FAILED_%', v_table_count;
  end if;

  select count(*) into v_pos_device_count
  from public.branch_devices
  where tenant_id = v_tenant_id and branch_id = v_branch_id and device_code = 'FG0004-POS-01' and status = 'inactive' and is_active = false;

  if v_pos_device_count <> 1 then
    raise exception 'FG0004_POS_DEVICE_POSTFLIGHT_FAILED_%', v_pos_device_count;
  end if;

  select count(*) into v_printer_slot_count
  from public.printer_profiles
  where tenant_id = v_tenant_id and branch_id = v_branch_id and printer_name in ('RECEIPT-01', 'KITCHEN-01') and enabled = false;
  ...
  if exists (select 1 from public.table_qr_sessions where tenant_id = v_tenant_id or branch_id = v_branch_id) then
    raise exception 'FG0004_QR_SESSION_SHOULD_NOT_EXIST';
  end if;

  if exists (select 1 from public.print_jobs where tenant_id = v_tenant_id or branch_id = v_branch_id) then
    raise exception 'FG0004_PRINT_JOB_SHOULD_NOT_EXIST';
  end if;
end $$;
