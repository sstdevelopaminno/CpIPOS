...
-- Guard the invariant when a scope is activated. A trial may never exceed 7 days
-- and retained business data may never exceed 30 days from the same start anchor.
create or replace function app.set_trial_scope_window(
  p_tenant_id uuid,
...
begin
  ...
  set lifecycle_status='trial',
      trial_started_at=p_started_at,
      trial_expires_at=p_started_at + interval '7 days',
      retention_until=p_started_at + interval '30 days',
      access_locked=false,
      lock_reason=null,
      updated_at=now()
...
-- Expiry locks access only. It never unlocks or extends a trial.
...
begin
  update public.trial_tenant_scopes
  set lifecycle_status='expired', access_locked=true, lock_reason='trial_expired', updated_at=now()
...
-- At retention end remove branch scopes. Their CASCADE graph removes Trial business data
-- while the tenant scope remains as a minimal audit/tombstone record.
...
begin
  with due as (
    select tenant_id
    from public.trial_tenant_scopes
    where retention_until is not null and retention_until <= now()
    for update
  ), deleted as (
    delete from public.trial_branch_scopes b
    using due d
  ...
  update public.trial_tenant_scopes
  set lifecycle_status='expired', access_locked=true, lock_reason='trial_data_retention_expired', updated_at=now(),
...
-- Retire BBQ completely from the Trial Data Plane. Deleting the tenant scope cascades to its branch/business rows.
delete from public.trial_tenant_scopes where tenant_code='BBQ-TH-002';

-- TEST-TH-003 is being converted into a real customer seed. Remove all copied QA business data
-- by deleting the branch scopes (CASCADE) but keep the tenant scope/UUID for manual provisioning.
do $$
declare v_tenant uuid;
begin
  select tenant_id into v_tenant from public.trial_tenant_scopes where tenant_code='TEST-TH-003';
  if v_tenant is not null then
    ...
    set lifecycle_status='trial',
        ...
        access_locked=true,
        lock_reason='awaiting_manual_trial_activation',
