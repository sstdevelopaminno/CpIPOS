-- CpIPOS Trial data-plane parity for authoritative POS dine-in cart replacement.
-- Adds the service-only replace_queued_dine_in_order_tx RPC and makes Kitchen
-- add/cancel tickets use kitchen_delta_quantity, matching Primary semantics.
...
declare
  ...
  v_event_hash text;
begin
  -- Trial has no authoritative public.shifts table. Validate the requested shift,
  -- branch, tenant and actor against a live Trial runtime lease instead.
  if not exists (
    select 1
    from public.trial_runtime_leases l
    where l.tenant_id = p_tenant_id
      and l.branch_id = p_branch_id
      and l.shift_id = p_shift_id
      and l.user_id = p_actor_user_id
      and l.status = 'active'
  ...
  insert into public.order_items(
    tenant_id, branch_id, order_id, product_id, quantity, unit_price, line_total, notes, name, metadata
  )
  select
    p_tenant_id, p_branch_id, p_order_id, ti.product_id, ti.quantity, ti.unit_price, ti.line_total,
    ti.notes, ti.product_name, jsonb_build_object('source', 'pos_dine_in_edit')
  from pg_temp.dine_in_target_items ti
  ...
  if coalesce(array_length(v_add_item_ids, 1), 0) > 0 then
    ...
    perform * from app.enqueue_kitchen_order(
  ...
  where oi.tenant_id = p_tenant_id
    ...
    and coalesce(oi.metadata->>'bill_line_state', 'active') <> 'cancelled'
    and not exists (
      select 1 from pg_temp.dine_in_target_items ti where ti.order_item_id = oi.id
    );
  ...
  set shift_id = p_shift_id,
      ...
      metadata = coalesce(o.metadata, '{}'::jsonb)
        || jsonb_build_object('tax_lines', coalesce(p_tax_lines, '[]'::jsonb), 'updated_from', 'pos_dine_in_rpc'),
      updated_at = now()
...
revoke all on function app.replace_queued_dine_in_order_tx(uuid,uuid,uuid,uuid,uuid,uuid,jsonb,numeric,numeric,numeric,numeric,numeric,jsonb)
  from public, anon, authenticated;
revoke all on function public.replace_queued_dine_in_order_tx(uuid,uuid,uuid,uuid,uuid,uuid,jsonb,numeric,numeric,numeric,numeric,numeric,jsonb)
  from public, anon, authenticated;
grant execute on function app.replace_queued_dine_in_order_tx(uuid,uuid,uuid,uuid,uuid,uuid,jsonb,numeric,numeric,numeric,numeric,numeric,jsonb)
  to service_role;
grant execute on function public.replace_queued_dine_in_order_tx(uuid,uuid,uuid,uuid,uuid,uuid,jsonb,numeric,numeric,numeric,numeric,numeric,jsonb)
  to service_role;

-- Trial enqueue parity: for POS cart increases/decreases, Kitchen receives only
-- the delta quantity, not the item's post-edit total quantity.
do $$
declare
  v_old text;
  v_new text;
  v_needle text := 'select oi.id order_item_id, oi.product_id, oi.quantity, oi.notes,';
  v_replacement text := 'select oi.id order_item_id, oi.product_id, case when p_action in (''add'',''cancel'') and oi.metadata ? ''kitchen_delta_quantity'' then greatest(0,(oi.metadata->>''kitchen_delta_quantity'')::numeric) else oi.quantity end quantity, oi.notes,';
begin
  select pg_get_functiondef('app.enqueue_kitchen_order(uuid,uuid,uuid,text,text,uuid[])'::regprocedure)
  into v_old;

  if position('kitchen_delta_quantity' in v_old) > 0 then
    return;
  end if;

  v_new := replace(v_old, v_needle, v_replacement);
  if v_new = v_old then
    raise exception 'TRIAL_KITCHEN_DELTA_PATCH_PATTERN_NOT_FOUND';
  end if;
  execute v_new;
end;
$$;
